...
if( ! class_exists ( 'Wt_Smart_Coupon_Common' ) ) {
    class Wt_Smart_Coupon_Common {
        ...
        public function update_existing_coupon_data_to_lookup_table()
        {
            ...
            $results = $wpdb->get_results("SELECT p.ID, p.post_status, p.post_date_gmt FROM {$wpdb->posts} AS p LEFT JOIN {$lookup_tb} AS l ON (p.ID=l.coupon_id) WHERE l.coupon_id IS NULL AND p.post_type = 'shop_coupon' ORDER BY p.ID ASC LIMIT 200", ARRAY_A);
            ...
            $results = $wpdb->get_results("SELECT p.ID FROM {$wpdb->posts} AS p LEFT JOIN {$lookup_tb} AS l ON (p.ID=l.coupon_id) WHERE l.coupon_id IS NULL AND p.post_type = 'shop_coupon' ORDER BY p.ID ASC LIMIT 1");
